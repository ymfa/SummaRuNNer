...
summary_filename = 'rnn_rnn.100.spl'  # change this to whatever you like

def untokenize(decoded_dir, filepath_pattern):
  in_paths = sorted(glob.glob(os.path.join(decoded_dir, '*.txt')))
  ...
  if len(out_paths) == 1 and len(in_paths) > 1:
    ...
    out_paths = [os.path.join(p, summary_filename) for p in out_paths]
  else:
